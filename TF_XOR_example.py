...
l_r = 0.5  # learning rate initialization
...
h3 = tf.sigmoid(z3)
cost_func = -tf.reduce_sum(tf.reduce_sum(y * tf.log(h3) + (1 - y) * tf.log(1 - h3), axis=1), axis=0)
# cost_func = tf.losses.mean_squared_error(labels=y, predictions=h3)
...
for i in range(10000):
    _, loss_val = sess.run([train, cost_func], feed_dict={x: X, y: Y})  # setting place holder values using feed_dict
    if i % 1000 == 0:
        print("Epoch:", i)
        print("Loss:", loss_val)

print(sess.run(h3, feed_dict={x: X, y: Y}))
